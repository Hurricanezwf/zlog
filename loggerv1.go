...
func (l *loggerv1) Fatalf(ctx context.Context, format string, v ...interface{}) {
	if l.level >= LevelFatal {
		l.logger.Output(2, l.format(ctx, "FATAL", fmt.Sprintf(format, v...)))
		os.Exit(1)
	}
}
...
func (l *loggerv1) Fatal(ctx context.Context, msg string) {
	if l.level >= LevelFatal {
		l.logger.Output(2, l.format(ctx, "FATAL", msg))
		os.Exit(1)
	}
}
...
func (l *loggerv1) format(ctx context.Context, levelFlag string, output string) string {
	if traceId := getTraceId(ctx); traceId != "" {
		return fmt.Sprintf(" %-5s --- [ %-6s | %-6s | %-36s ] :   %s", levelFlag, getServiceName(ctx), getModuleName(ctx), traceId, output)
	}
	return fmt.Sprintf(" %-5s --- [ %-6s | %-6s ] :   %s", levelFlag, getServiceName(ctx), getModuleName(ctx), output)
}
